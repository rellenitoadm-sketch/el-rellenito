                ...
                <div className="flex-1 overflow-y-auto">
                  {items.length === 0 ? (
                    ...
                    <div className="px-4 mt-4 space-y-3">
                      {items.map(item => (
                        <div key={item.id} className="flex items-center gap-3 rounded-xl p-3 border" style={{ background: 'var(--surface-2)', borderColor: 'var(--border)' }}>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold line-clamp-1" style={{ color: 'var(--text-primary)' }}>{item.name}</p>
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <button
                              ...
                              className="w-7 h-7 rounded-full flex items-center justify-center transition-colors hover:bg-[var(--brand-orange)] hover:text-white" style={{ background: 'var(--surface-3)', color: 'var(--text-secondary)' }}
                            ...
                            <button
                              onClick={() => updateQty(item.id, item.quantity + 1)}
                              className="w-7 h-7 rounded-full bg-[#FF5100] flex items-center justify-center text-white hover:bg-[#e04800] transition-colors"
                ...
                {items.length > 0 && (
                  <div className="px-4 pb-6 pt-3 border-t" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
                    <div className="flex items-center justify-between mb-3 px-1">
                      ...
                      <span className="text-lg font-bold text-[#FF5100]">{format(totalUsd)}</span>
                    </div>
                    <button
